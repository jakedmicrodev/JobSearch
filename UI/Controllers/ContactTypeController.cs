using Business.Entities;
using Business.Services;
using System.Collections.Generic;
using System.Web.Mvc;

namespace UI.Controllers
{
    public class ContactTypeController : Controller
    {
        // GET: ContactType
        public ActionResult Index()
        {
            List<IdName> v = new JobSearchService().GetContactTypes();
            return View(v);
        }

        // GET: ContactType/Details/5
        public ActionResult Details(int id)
        {
            IdName a = new JobSearchService().GetContactType(id);
            return View(a);
        }

        // GET: ContactType/Create
        public ActionResult Create()
        {
            IdName a = new JobSearchService().GetContactType();
            return View(a);
        }

        // POST: ContactType/Create
        [HttpPost]
        public ActionResult Create(IdName a)
        {
            try
            {
                JobSearchService m = new JobSearchService();
                m.CreateContactType(a);

                return RedirectToAction("Index");
            }
            catch
            {
                return View();
            }
        }

        // GET: ContactType/Edit/5
        public ActionResult Edit(int id)
        {
            IdName a = new JobSearchService().GetContactType(id);
            return View(a);
        }

        // POST: ContactType/Edit/5
        [HttpPost]
        public ActionResult Edit(IdName a)
        {
            try
            {
                JobSearchService m = new JobSearchService();
                m.EditContactType(a);

                return RedirectToAction("Index");
            }
            catch
            {
                return View();
            }
        }

        // GET: ContactType/Delete/5
        public ActionResult Delete(int id)
        {
            return View();
        }

        // POST: ContactType/Delete/5
        [HttpPost]
        public ActionResult Delete(int id, FormCollection collection)
        {
            try
            {
                // TODO: Add delete logic here

                return RedirectToAction("Index");
            }
            catch
            {
                return View();
            }
        }
    }
}
